// // app/api/tasks/create-posting-tasks/route.ts (POST only)
...
//     // Pull ALL existing names under this assignment once to normalize
//     const existingCopies = await prisma.task.findMany({
//       where: { assignmentId: assignment.id },
//       select: { name: true, templateSiteAssetId: true },
//     });
...
//       if (skipNameBaseSet.has(baseName) || skipPairBase.has(pairKey)) continue;
...
//       if (skipNameBaseSet.has(scBase) || skipPairBase.has(pairKey)) continue;
...
//       if (skipNameBaseSet.has(normalizeForDedupe(scName))) continue;
