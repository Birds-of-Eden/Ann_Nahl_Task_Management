...
// inclusive: counts start & end months if overlap
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Source (qc_approved) + type filter
    ...
    // ===== Build remaining schedule per-source with per-month cap and extension after dueDate =====
    type FutureItem = {
    ...
    for (const src of sourceTasks) {
      ...
      // fetch existing items for this base to know (1) per-month used, (2) max seq
      const existingForBase = await prisma.task.findMany({
        where: {
          assignmentId: assignment.id,
          name: { startsWith: `${base} -` },
          category: { name: { in: [CAT_SOCIAL_ACTIVITY, CAT_BLOG_POSTING] } },
        },
      ...
      let maxSeq = 0;
      for (const t of existingForBase) {
        const d = t.dueDate ? new Date(t.dueDate) : null;
        if (d) {
          const key = monthKey(d);
          existingByMonth.set(key, (existingByMonth.get(key) ?? 0) + 1);
        }
      ...
      const createdThisRunByMonth = new Map<string, number>();
      let accepted = 0;
      let seqCursor = maxSeq; // continue numbering after existing

      for (const d of cadenceDates(startDate)) {
        if (d <= todayMidnight) continue; // only after "today"

        // first try to fulfill months up to dueDate respecting cap
        if (d <= dueDate) {
          const key = monthKey(d);
          const already = (existingByMonth.get(key) ?? 0) + (createdThisRunByMonth.get(key) ?? 0);
          if (already >= freqPerMonth) {
            continue; // cap reached for this calendar month
          }

          // accept
          createdThisRunByMonth.set(key, (createdThisRunByMonth.get(key) ?? 0) + 1);
          accepted++;
          seqCursor++;
        ...
        // if dueDate passed and we still haven't met totalNeeded, extend after dueDate
    ...
    if (future.length === 0) {
      return NextResponse.json(
        {
          message: "No remaining occurrences to create (caps already satisfied).",
          created: 0,
          assignedTo: topAgent,
          tasks: [],
        },
    ...
    // Skip duplicates by name across categories
    ...
    // Social Communication (once per social base) aligned to latest created date in this run
    ...
    const latestDue =
      createdDates.length > 0
        ...
        : addWorkingDays(todayMidnight, 7);
    ...
    return NextResponse.json(
      {
        message: `Created ${created.length} remaining task(s) with per-month caps and assigned to ${topAgent.name}.`,
        created: created.length,
        assignedTo: topAgent,
        assignmentId: assignment.id,
        cadence: "first at startDate + 15 working days, then every +7 working days (per-month capped; spillover extends past dueDate)",
        tasks: created,
      },
      { status: 201 }
    );
