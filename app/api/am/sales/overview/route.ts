...
export async function GET() {
  ...
  for (const c of enriched) {
    if (!c.startDate || !c.dueDate) { missingDates++; continue; }
  ...
  const summary = {
    totalWithPackage, active, expired, startingSoon, expiringSoon, missingDates,
  };

  // --- Group by package (counts, active/expired, avg days left)
  const byPackageMap = new Map<string, {
    packageId: string; packageName: string | null; totalMonths: number | null;
    clients: number; active: number; expired: number; daysLeftAcc: number; daysLeftCount: number;
  }>();
  ...
  for (const c of enriched) {
    ...
    const e = byPackageMap.get(key) ?? {
      ...
      clients: 0, active: 0, expired: 0, daysLeftAcc: 0, daysLeftCount: 0,
    };
    e.clients++;
    if (c.startDate && c.dueDate) {
      ...
      e.daysLeftAcc += daysLeft; e.daysLeftCount++;
    }
  ...
  const byPackage = Array.from(byPackageMap.values())
    .map(e => ({
      ...
      active: e.active,
      expired: e.expired,
      avgDaysLeft: e.daysLeftCount ? Math.round(e.daysLeftAcc / e.daysLeftCount) : null,
    }))
    .sort((a, b) => b.clients - a.clients);

  // --- Timeseries: new package starts per day (last 90 days)
  const since = new Date(); since.setDate(since.getDate() - 90);
  const timeseries = await prisma.$queryRawUnsafe<{ day: string; starts: number }[]>(
    `
      SELECT
  ...
  const groupedClients = byPackage.map(pkg => ({
    ...
    clients: enriched
      ...
      .map(c => ({
        ...
        startDate: c.startDate, dueDate: c.dueDate, status: c.status,
      })),
  }));
